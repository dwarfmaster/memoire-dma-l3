...
\begin{proof} Induction sur la structure de la formule.
\end{proof}
...
\begin{lem}
    ...
    % \[\begin{tikzcd}
    %     \C_\Sigma(\star,[\phi]) \arrow[d, "f"] & \V(\phi) \arrow[l, "[.]"]
    %                                                       \arrow[d, "\iota"] \\
...
\begin{defi}{Sémantique d'une formule}
    ...
    dans $\V(\psi)$ d'après le lemme ?? % TODO numéroter les lemmes et faire les liens
...
\begin{defi}{Faisceau de CSP}
    ...
    $v^*$ de manière naturelle dans $\V(\psi_{(r,\alpha)})$ d'après le lemme %TODO ref
    . Alors on pose~:
